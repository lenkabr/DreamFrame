...
export default function Home() {
  ...
  return <main>
    ...
    {film && <section className="result-wrap" id="recommendation" aria-live="polite">
      <div className="result-label"><span>Tonight’s film</span><i /></div>
      <article className="film-card">
        <div className={`poster ${film.palette}`} role="img" aria-label={`Stylized poster for ${film.title}`}><span className="poster-title">{film.mark.split('\n').map((line) => <b key={line}>{line}</b>)}</span><span className="poster-credit">A film chosen for this moment</span></div>
        <div className="film-copy">
          <div className="film-heading"><div><p className="meta">{film.year} <span>•</span> {film.runtime}</p><h2>{film.title}</h2></div><div className="genres">{film.genres.map((genre) => <span key={genre}>{genre}</span>)}</div></div>
